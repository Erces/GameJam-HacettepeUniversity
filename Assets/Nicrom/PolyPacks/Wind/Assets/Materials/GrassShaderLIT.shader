...
Shader "GrassShaderLIT"
{
	Properties
	{
		...
		_Color0("Color 0", Color) = (0.5726988,1,0.3915094,1)
		_Color1("Color 1", Color) = (1,0.1273585,0.1273585,1)
		_GradientPowerMultiplier("Gradient Power Multiplier", Vector) = (2,2,0,0)
		_AlphaClipThresold("Alpha Clip Thresold", Range( 0 , 1)) = 0.54
		_Float1("Float 1", Float) = 2
		_Float0("Float 0", Float) = 2
		_DistanceExtra("Distance Extra", Float) = 1
		[ASEEnd]_ShadowAlphaClip("Shadow Alpha Clip", Range( 0 , 3)) = 0.57
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Color0;
			float4 _Color1;
			float2 _GradientPowerMultiplier;
			float _Float0;
			float _Float1;
			float _DistanceExtra;
			float _AlphaClipThresold;
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float3 break25 = inputMesh.positionOS;
				float4 appendResult26 = (float4(break25.x , ( break25.y + ( sin( ( ( break25.x * _Float0 ) + _TimeParameters.x ) ) * _Float1 ) ) , break25.z , 0.0));
				float2 texCoord36 = inputMesh.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult35 = lerp( float4( inputMesh.positionOS , 0.0 ) , appendResult26 , texCoord36.y);
				
				float3 vertexPos37 = inputMesh.positionOS;
				float4 ase_clipPos37 = TransformWorldToHClip( TransformObjectToWorld(vertexPos37));
				float4 screenPos37 = ComputeScreenPos( ase_clipPos37 , _ProjectionParams.x );
				outputPackedVaryingsMeshToPS.ase_texcoord6 = screenPos37;
						...
						)
			{
				...
				float2 texCoord12 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult21 = lerp( _Color0 , _Color1 , ( 1.0 - saturate( ( pow( texCoord12.y , _GradientPowerMultiplier.x ) * _GradientPowerMultiplier.y ) ) ));
				float4 tex2DNode13 = tex2D( _TextureSample0, texCoord12 );
				
				float4 screenPos37 = packedInput.ase_texcoord6;
				float4 ase_screenPosNorm37 = screenPos37 / screenPos37.w;
				ase_screenPosNorm37.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm37.z : ase_screenPosNorm37.z * 0.5 + 0.5;
				float3 objToWorld42 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord7.xyz, 1 ) ).xyz);
				float clampResult45 = clamp( ( distance( _WorldSpaceCameraPos , objToWorld42 ) * _DistanceExtra ) , 0.0 , 1.0 );
				float screenDepth37 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm37.xy ),_ZBufferParams);
				float distanceDepth37 = saturate( abs( ( screenDepth37 - LinearEyeDepth( ase_screenPosNorm37.z,_ZBufferParams ) ) / ( clampResult45 ) ) );
				
				surfaceDescription.Albedo = ( lerpResult21 * tex2DNode13 ).rgb;
				...
				surfaceDescription.Alpha = ( tex2DNode13.a * distanceDepth37 );

				#ifdef _ALPHATEST_ON
				surfaceDescription.AlphaClipThreshold = _AlphaClipThresold;
				#endif

				#ifdef _ALPHATEST_SHADOW_ON
				surfaceDescription.AlphaClipThresholdShadow = _ShadowAlphaClip;
				#endif
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float3 break25 = inputMesh.positionOS;
				float4 appendResult26 = (float4(break25.x , ( break25.y + ( sin( ( ( break25.x * _Float0 ) + _TimeParameters.x ) ) * _Float1 ) ) , break25.z , 0.0));
				float2 texCoord36 = inputMesh.uv0.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult35 = lerp( float4( inputMesh.positionOS , 0.0 ) , appendResult26 , texCoord36.y);
				
				float3 vertexPos37 = inputMesh.positionOS;
				float4 ase_clipPos37 = TransformWorldToHClip( TransformObjectToWorld(vertexPos37));
				float4 screenPos37 = ComputeScreenPos( ase_clipPos37 , _ProjectionParams.x );
				outputPackedVaryingsMeshToPS.ase_texcoord1 = screenPos37;
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 screenPos37 = packedInput.ase_texcoord1;
				float4 ase_screenPosNorm37 = screenPos37 / screenPos37.w;
				ase_screenPosNorm37.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm37.z : ase_screenPosNorm37.z * 0.5 + 0.5;
				float3 objToWorld42 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord2.xyz, 1 ) ).xyz);
				float clampResult45 = clamp( ( distance( _WorldSpaceCameraPos , objToWorld42 ) * _DistanceExtra ) , 0.0 , 1.0 );
				float screenDepth37 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm37.xy ),_ZBufferParams);
				float distanceDepth37 = saturate( abs( ( screenDepth37 - LinearEyeDepth( ase_screenPosNorm37.z,_ZBufferParams ) ) / ( clampResult45 ) ) );
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float4 ase_clipPos37 = TransformWorldToHClip( TransformObjectToWorld(vertexPos37));
				float4 screenPos37 = ComputeScreenPos( ase_clipPos37 , _ProjectionParams.x );
				outputPackedVaryingsMeshToPS.ase_texcoord2 = screenPos37;
					...
					)
			{
				...
				float4 tex2DNode13 = tex2D( _TextureSample0, texCoord12 );
				float4 screenPos37 = packedInput.ase_texcoord2;
				float4 ase_screenPosNorm37 = screenPos37 / screenPos37.w;
				ase_screenPosNorm37.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm37.z : ase_screenPosNorm37.z * 0.5 + 0.5;
				float3 objToWorld42 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord3.xyz, 1 ) ).xyz);
				float clampResult45 = clamp( ( distance( _WorldSpaceCameraPos , objToWorld42 ) * _DistanceExtra ) , 0.0 , 1.0 );
				float screenDepth37 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm37.xy ),_ZBufferParams);
				float distanceDepth37 = saturate( abs( ( screenDepth37 - LinearEyeDepth( ase_screenPosNorm37.z,_ZBufferParams ) ) / ( clampResult45 ) ) );
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float4 ase_clipPos37 = TransformWorldToHClip( TransformObjectToWorld(vertexPos37));
				float4 screenPos37 = ComputeScreenPos( ase_clipPos37 , _ProjectionParams.x );
				outputPackedVaryingsMeshToPS.ase_texcoord4 = screenPos37;
					...
					)
			{
				...
				float2 texCoord12 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode13 = tex2D( _TextureSample0, texCoord12 );
				float4 screenPos37 = packedInput.ase_texcoord4;
				float4 ase_screenPosNorm37 = screenPos37 / screenPos37.w;
				ase_screenPosNorm37.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm37.z : ase_screenPosNorm37.z * 0.5 + 0.5;
				float3 objToWorld42 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord5.xyz, 1 ) ).xyz);
				float clampResult45 = clamp( ( distance( _WorldSpaceCameraPos , objToWorld42 ) * _DistanceExtra ) , 0.0 , 1.0 );
				float screenDepth37 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm37.xy ),_ZBufferParams);
				float distanceDepth37 = saturate( abs( ( screenDepth37 - LinearEyeDepth( ase_screenPosNorm37.z,_ZBufferParams ) ) / ( clampResult45 ) ) );
		...
		Pass
		{
			...
			AttributesMesh ApplyMeshModification(AttributesMesh inputMesh, float3 timeParameters, inout PackedVaryingsMeshToPS outputPackedVaryingsMeshToPS )
			{
				...
				float4 ase_clipPos37 = TransformWorldToHClip( TransformObjectToWorld(vertexPos37));
				float4 screenPos37 = ComputeScreenPos( ase_clipPos37 , _ProjectionParams.x );
				outputPackedVaryingsMeshToPS.ase_texcoord8 = screenPos37;
				...
				)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord12 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float4 lerpResult21 = lerp( _Color0 , _Color1 , ( 1.0 - saturate( ( pow( texCoord12.y , _GradientPowerMultiplier.x ) * _GradientPowerMultiplier.y ) ) ));
				float4 tex2DNode13 = tex2D( _TextureSample0, texCoord12 );
				
				float4 screenPos37 = packedInput.ase_texcoord8;
				float4 ase_screenPosNorm37 = screenPos37 / screenPos37.w;
				ase_screenPosNorm37.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm37.z : ase_screenPosNorm37.z * 0.5 + 0.5;
				float3 objToWorld42 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord9.xyz, 1 ) ).xyz);
				float clampResult45 = clamp( ( distance( _WorldSpaceCameraPos , objToWorld42 ) * _DistanceExtra ) , 0.0 , 1.0 );
				float screenDepth37 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm37.xy ),_ZBufferParams);
				float distanceDepth37 = saturate( abs( ( screenDepth37 - LinearEyeDepth( ase_screenPosNorm37.z,_ZBufferParams ) ) / ( clampResult45 ) ) );
...
/*ASEBEGIN
Version=18933
0;73;961;625;123.7267;-55.65651;1.054265;True;False
Node;AmplifyShaderEditor.PosVertexDataNode;24;-1976.592,891.5347;Inherit;False;0;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;28;-1211.206,960.6884;Inherit;False;Property;_Float0;Float 0;6;0;Create;True;0;0;0;False;0;False;2;2;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.BreakToComponentsNode;25;-1055.205,701.9885;Inherit;True;FLOAT3;1;0;FLOAT3;0,0,0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;27;-836.8036,812.4883;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector2Node;18;-1505.397,274.3139;Inherit;False;Property;_GradientPowerMultiplier;Gradient Power Multiplier;3;0;Create;True;0;0;0;False;0;False;2,2;1,1;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
Node;AmplifyShaderEditor.TextureCoordinatesNode;12;-1411.502,36.79998;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleTimeNode;30;-801.7036,1021.788;Inherit;False;1;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;29;-595.004,844.9884;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.PowerNode;17;-1207.825,191.0786;Inherit;True;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.WorldSpaceCameraPos;39;-1972.972,703.3888;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.TransformPositionNode;42;-1638.668,973.0127;Inherit;True;Object;World;False;Fast;True;1;0;FLOAT3;0,0,0;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SinOpNode;31;-371.4041,833.2888;Inherit;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.DistanceOpNode;40;-1638.884,595.3;Inherit;True;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;33;-316.8043,1137.488;Inherit;False;Property;_Float1;Float 1;5;0;Create;True;0;0;0;False;0;False;2;2;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;19;-1005.725,267.1786;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;44;-1476.574,742.8495;Inherit;False;Property;_DistanceExtra;Distance Extra;7;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;43;-1313.15,648.9274;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SaturateNode;20;-882.3008,287.2876;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;32;-132.2043,869.6885;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ClampOpNode;45;-1145.967,575.6669;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;11;-1078.598,-140.5002;Inherit;False;Property;_Color0;Color 0;1;0;Create;True;0;0;0;False;0;False;0.5726988,1,0.3915094,1;0.5726988,1,0.3915094,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;22;-1070.806,31.18801;Inherit;False;Property;_Color1;Color 1;2;0;Create;True;0;0;0;False;0;False;1,0.1273585,0.1273585,1;1,0.1273585,0.1273585,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.PosVertexDataNode;41;-915.9707,381.2308;Inherit;False;0;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.OneMinusNode;23;-783.5049,220.9879;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;34;-4.804274,822.8893;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.DynamicAppendNode;26;160.2952,703.2885;Inherit;False;FLOAT4;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.TextureCoordinatesNode;36;134.8266,908.6595;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.LerpOp;21;-661.3014,88.38751;Inherit;True;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.DepthFade;37;-757.3529,556.7397;Inherit;False;True;True;True;2;1;FLOAT3;0,0,0;False;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;13;-540.1991,286.6999;Inherit;True;Property;_TextureSample0;Texture Sample 0;0;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;46;415.0028,480.5253;Inherit;False;Property;_ShadowAlphaClip;Shadow Alpha Clip;8;0;Create;True;0;0;0;False;0;False;0.57;0;0;3;0;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;35;346.1951,586.2885;Inherit;False;3;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;2;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;38;-244.8646,518.7776;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;14;-317.1999,37.69999;Inherit;True;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;15;-83.36974,452.4811;Inherit;False;Property;_AlphaClipThresold;Alpha Clip Thresold;4;0;Create;True;0;0;0;False;0;False;0.54;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;16;-121,177.5;Inherit;False;Constant;_zero;zero;2;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;0;618,147.5;Float;False;True;-1;2;Rendering.HighDefinition.LightingShaderGraphGUI;0;2;GrassShaderLIT;53b46d85872c5b24c8f4f0a1c3fe4c87;True;GBuffer;0;0;GBuffer;35;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;3;RenderPipeline=HDRenderPipeline;RenderType=Transparent=RenderType;Queue=Transparent=Queue=0;True;5;True;7;d3d11;metal;vulkan;xboxone;xboxseries;playstation;switch;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;0;True;-27;False;False;False;False;False;False;False;False;False;True;True;0;True;-14;255;False;-1;255;True;-13;7;False;-1;3;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;False;False;True;0;True;-15;False;True;1;LightMode=GBuffer;False;False;0;;0;0;Standard;42;Surface Type;1;637767513688233707;  Rendering Pass;1;637767658387843163;  Refraction Model;0;0;    Blending Mode;0;637767652016740134;    Blend Preserves Specular;0;637767652064836218;  Receive Fog;0;637767652055356699;  Back Then Front Rendering;1;637767651423408518;  Transparent Depth Prepass;0;0;  Transparent Depth Postpass;0;637767632064034342;  Transparent Writes Motion Vector;0;0;  Distortion;0;0;    Distortion Mode;0;0;    Distortion Depth Test;1;0;  ZWrite;0;0;  Z Test;4;0;Double-Sided;1;637767651565877151;Alpha Clipping;1;637767513881224334;  Use Shadow Threshold;1;637767669221454547;Material Type,InvertActionOnDeselection;0;0;  Energy Conserving Specular;1;0;  Transmission;1;0;Receive Decals;0;637767513003775783;Receives SSR;1;0;Receive SSR Transparent;0;0;Motion Vectors;0;637767669387282871;  Add Precomputed Velocity;0;0;Specular AA;0;0;Specular Occlusion Mode;1;637767659622631526;Override Baked GI;0;0;Depth Offset;0;0;DOTS Instancing;0;0;LOD CrossFade;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,-1;0;  Type;0;0;  Tess;16,False,-1;0;  Min;10,False,-1;0;  Max;25,False,-1;0;  Edge Length;16,False,-1;0;  Max Displacement;25,False,-1;0;Vertex Position;1;0;0;11;True;True;True;True;True;True;False;True;False;False;True;False;;False;0
...
WireConnection;0;30;46;0
WireConnection;0;11;35;0
ASEEND*/
//CHKSM=A0ADF1F76F47FFEADFF1477D0A6258B499C295B2